function initPageListTable(page_config) {
	if(!page_config.toolbar){
		page_config.toolbar = '#toolbar';
	}
	if(page_config.showColumns == undefined){
		page_config.showColumns = true;
	}
	
	if(page_config.showRefresh == undefined){
		page_config.showRefresh = true;
	}
    ...
    $("#"+page_config.tableId).bootstrapTable({ // 对应table标签的id
        ...
        toolbar:page_config.toolbar,//工具栏
        showRefresh: page_config.showRefresh,//显示刷新按钮
        showColumns: page_config.showColumns,//显示列选择
